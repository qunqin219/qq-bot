...
自定义兼容端点既可以填写服务根地址，也可以填写以 `/v1` 结尾的地址；运行时统一调用 `/v1/responses`。请求使用 `stream: true`，服务端会把 Responses SSE 增量事件还原为完整回复，因此也兼容强制流式输出的中转网关。OpenAI 工具调用会保留 `call_id`，并继续复用本项目的 Agent 权限、审批、上下文压缩和持久化运行记录。
...
OneBot event -> agent selection -> context budget -> model/tool loop
             -> permission policy -> approval/tool execution -> QQ reply
```

内置 Agent：

- `assistant`：聊天、图片、记忆和只读查询。
- `group-manager`：群成员与群管理任务；写操作默认需要审批。
- `summarizer`：无工具的会话压缩角色。

每次运行、工具调用、工具结果、压缩摘要和审批都会持久化。SQLite 模式写入 `data/bot.db`；JSON 兼容模式写入 `data/agent-runtime.json`。服务重启时，未完成运行会被标记为 `interrupted`。

危险工具支持 `allow`、`ask`、`deny` 策略。默认群管理写操作为 `ask`：管理员可以在原消息里写“确认执行/确认禁言”等直接授权，也可以在收到审批 ID 后发送：
...
`agent_tool_permissions` 也支持 `group:<群号>` 覆盖：
