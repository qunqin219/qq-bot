...
async function buildGroupAwarePrompt(event, client, cfg, currentMsg, managementContext = null) {
  ...
  const sections = [
    '以下是当前 QQ 群聊上下文，仅用于理解用户这次 @Bot 的问题。不要主动复述上下文；如果上下文不足，再简短追问。最近群聊消息里带有“消息ID=数字”和“QQ=数字”；如果某条消息是发给你的，会标成“对Bot说”，@ 信息会保留成 @Bot 或 @QQ=数字。当前会话有持久化聊天记录检索工具 qq_search_chat_history，支持关键词、发言人QQ和时间范围过滤，query 可以为空。凡是用户要求查询、搜索、回顾、统计、分析聊天记录、某人发言、某话题历史、上下文来源，或要求基于“我/某人/本群”的历史发言做判断时，应先调用这个工具，而不是只凭最近上下文猜；提到“我/我的发言”时用当前触发用户 QQ 作为 user_id。普通闲聊、无需历史证据的问题不要调用。只有当用户明确要求引用、回复、评价某个人/某条消息，或问题里明显用“他/她/那条/上面那条”指向某条上文时，才选择消息ID，并在最终回复第一行输出“引用消息ID：数字”，第二行开始写正文。普通追问、继续、还有吗、闲聊时不要输出引用消息ID。这个标记是给系统看的，不要解释。',
  ];
